...
public class Solution {
    ...
    private List<List<Integer>> findAncestorsPerNodeWithTopologicalSort(List<Integer>[] directedAcyclicGraph, int[] incomingEdgesPerNode) {
        ...
        boolean[][] ancestors = new boolean[numberOfNodes][numberOfNodes];
    ...
    private List<List<Integer>> createListAllAncestorsPerNode(boolean[][] ancestors) {
    ...
    private void updateAncestors(int current, int next, boolean[][] ancestors) {
        for (int nodeID = 0; nodeID < numberOfNodes; ++nodeID) {
            ancestors[next][nodeID] = ancestors[next][nodeID] || ancestors[current][nodeID];
        }
        ancestors[next][current] = true;
    }
}
